fn solve(n: usize, w: u64, a: Vec<u64>) -> bool {
...
fn _solve(i: usize, w: u64, a: &Vec<u64>, dp: &mut Vec<Vec<Option<bool>>>) -> bool {
    ...
    if _solve(i - 1, w, a, dp) {
        dp[i][w as usize] = Some(true);
        return true;
    }

    if _solve(i - 1, w - a[i - 1], a, dp) {
        dp[i][w as usize] = Some(true);
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn test_1() {
        assert_eq!(solve(4, 5, vec![1, 2, 4, 8]), true);
    }

    #[test]
    fn test_2() {
        assert_eq!(solve(4, 16, vec![1, 2, 4, 8]), false);
    }
}
